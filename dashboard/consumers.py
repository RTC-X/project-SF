    ...
    def update_bot_status(self, username, status, extra_data=None):
        ...
        try:
            ...
            if extra_data:
                ...
                try:
                    if 'BankLevel' in extra_data and extra_data['BankLevel'] is not None:
                        bot.bank_level = int(extra_data['BankLevel'])
                except Exception as e:
                    print(f"[C2 Server] [DB Defend] Failed to set bank_level (unmigrated?): {e}")

                try:
                    if 'AscenderLevel' in extra_data and extra_data['AscenderLevel'] is not None:
                        bot.ascender_level = int(extra_data['AscenderLevel'])
                except Exception as e:
